...
export async function getAppUsage(
  ...
  try {
    ...
    // Aggregate by app name, summing durations
    const appMap = new Map<string, number>();
    for (const event of events) {
      const app = event.data.app;
      if (app && typeof app === "string") {
        const existing = appMap.get(app) ?? 0;
        appMap.set(app, existing + event.duration);
      }
    }

    // Convert to AppUsage[] and sort by duration descending
    const result: AppUsage[] = Array.from(appMap, ([app, duration]) => ({
      app,
      duration,
